...
public class UpdateStatus extends AppCompatActivity implements OnMapReadyCallback {
    ...
    private String deliveryLat, deliveryLongi, pickupLat, pickupLongi,statusString;
    private int totalDeliveryCost, quantity, totalCostWithDeliveryCharge, productCostAmout;
    private ImageView gMapButton;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        try {
            pickupLat = deliveryInfo.getSender_lat();
            pickupLongi = deliveryInfo.getSender_longi();
            pickupLocation = new MarkerOptions().position(new LatLng(Double.parseDouble(pickupLat), Double.parseDouble(pickupLongi))).title("Pickup Location").icon(BitmapDescriptorFactory.defaultMarker(BitmapDescriptorFactory.HUE_YELLOW));
        }
        catch (Exception e)
        {
            Intent intent = new Intent(getApplicationContext(), ShowDetailsWithoutMap.class);
            intent.putExtra(Constants.DELIVERY_OBJECT, deliveryInfo);
            intent.putExtra(Constants.DELIVERY_MAN_OBJECT, deliveryManInfo);
            startActivity(intent);
            Toast.makeText(this,"pickup address is invalid",Toast.LENGTH_SHORT).show();
            Log.e("UpdateStatus",e.toString());
            finish();
        }
        try {

            deliveryLat = deliveryInfo.getReceiver_lat();
            deliveryLongi = deliveryInfo.getReceiver_longi();
            deliveryLocation = new MarkerOptions().position(new LatLng(Double.parseDouble(deliveryLat), Double.parseDouble(deliveryLongi))).title("Delivery Location").icon(BitmapDescriptorFactory.defaultMarker(BitmapDescriptorFactory.HUE_RED));
        }
        catch (Exception e)
        {
            Intent intent = new Intent(getApplicationContext(), ShowDetailsWithoutMap.class);
            ...
            intent.putExtra(Constants.DELIVERY_MAN_OBJECT, deliveryManInfo);
            startActivity(intent);
            Toast.makeText(this,"delivery address is invalid",Toast.LENGTH_SHORT).show();
            Log.e("UpdateStatus",e.toString());
            finish();
        }
        ...
        gMapButton.setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View view) {
                if(statusString.equals(Constants.JUST_CREATED)||statusString.equals(Constants.ENROUTE_TO_PICKUP)) {
                    String destination="http://maps.google.com/maps?&daddr="+pickupLat+","+pickupLongi;
                    Intent intend = new Intent(Intent.ACTION_VIEW, Uri.parse(destination));
                    startActivity(intend);
                }
                else{
                    String destination="http://maps.google.com/maps?&daddr="+deliveryLat+","+deliveryLongi;
                    Intent intend = new Intent(Intent.ACTION_VIEW, Uri.parse(destination));
                    startActivity(intend);
                }
    ...
    public void getDeliveryStatusForDeliveryMan(){
        ...
        call.enqueue(new Callback<DeliveryStatus>() {
            ...
            public void onFailure(Call<DeliveryStatus> call, Throwable t) {
                Toast.makeText(UpdateStatus.this,t.getMessage().toString(),Toast.LENGTH_LONG).show();
            }
        });
    }
    ...
    public void updateDeliveryStatus(String deliveryId, final String status){
        ...
        call.enqueue(new Callback<DeliveryStatus>() {
            ...
            public void onResponse(Call<DeliveryStatus> call, Response<DeliveryStatus> response) {
                ...
                {
                    Toast.makeText(UpdateStatus.this,"unsuccessful response",Toast.LENGTH_SHORT).show();
                }
            }
            @Override
            public void onFailure(Call<DeliveryStatus> call, Throwable t) {
    ...
    public void onMapReady(GoogleMap gmap) {
        googleMap = gmap;
        googleMap.addMarker(pickupLocation);
        ...
        if(statusString.equals(Constants.JUST_CREATED)||statusString.equals(Constants.ENROUTE_TO_PICKUP)) {
            camera = new CameraPosition.Builder()
                    .target(new LatLng(pickupLocation.getPosition().latitude, pickupLocation.getPosition().longitude))
                    .zoom(15)
                    .bearing(0)
                    .tilt(45)
                    .build();
        ...
        {
            camera = new CameraPosition.Builder()
                    .target(new LatLng(deliveryLocation.getPosition().latitude, deliveryLocation.getPosition().longitude))
                    .zoom(15)
                    .bearing(0)
                    .tilt(45)
                    .build();
